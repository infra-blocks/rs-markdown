...
pub mod open {
    ...
    impl<'a> IOpenBlock<'a> for BackticksFencedCode<'a> {
        ...
        fn close(self) -> Self::Closed {
            super::BackticksFencedCode::new(self.opening, self.content, self.closing)
        }
    }
    ...
    mod test {
        ...
        use crate::parse::blocks::{
            block::leaf::fenced_code::backticks::parse::opening, open_block::IBlockTestExt,
        };
        ...
        fn should_should_reject_staging_after_closing() {
            let mut block = Block::strict_open_and_commit_all("```rust\n```");
        ...
        #[test]
        fn should_work_when_closed_with_staged_content() {
        ...
        fn should_work_when_closed_after_comitting_staged_content() {
            let block = Block::strict_open_and_commit_all("```rust\nlet x = 42;\n");
            assert_eq!(
                Block::new(
        ...
        #[test]
        fn should_work_when_closed_with_comitting_closing() {
            let block = Block::strict_open_and_commit_all("```rust\n```\n");
            assert_eq!(
                Block::new(
...
mod parse {
    ...
    impl<'a> Default for ContentOrClosing<'a> {
        fn default() -> Self {
            ContentOrClosing::Content("")
        }
    }
