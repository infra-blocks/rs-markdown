...
use case_1::HtmlCase1;
use case_2::HtmlCase2;
use case_3::HtmlCase3;
use case_4::HtmlCase4;
use case_5::HtmlCase5;
// use case_6::HtmlCase6;
...
impl<'a> From<HtmlCase1<'a>> for Html<'a> {
    fn from(case: HtmlCase1<'a>) -> Self {
        Self::Case1(case)
    }
}

impl<'a> From<HtmlCase2<'a>> for Html<'a> {
    fn from(case: HtmlCase2<'a>) -> Self {
        Self::Case2(case)
    }
}

impl<'a> From<HtmlCase3<'a>> for Html<'a> {
    fn from(case: HtmlCase3<'a>) -> Self {
        Self::Case3(case)
    }
}

impl<'a> From<HtmlCase4<'a>> for Html<'a> {
    fn from(case: HtmlCase4<'a>) -> Self {
        Self::Case4(case)
    }
}

impl<'a> From<HtmlCase5<'a>> for Html<'a> {
    fn from(case: HtmlCase5<'a>) -> Self {
        Self::Case5(case)
    }
}

// impl<'a> From<HtmlCase6<'a>> for Html<'a> {
//     fn from(case: HtmlCase6<'a>) -> Self {
//         Self::Case6(case)
//     }
// }
...
pub mod open {
    use crate::parse::blocks::{
        block::leaf::html::{
            case_1::open::HtmlCase1, case_2::open::HtmlCase2, case_3::open::HtmlCase3,
            case_4::open::HtmlCase4, case_5::open::HtmlCase5,
        },
    ...
    impl<'a> IOpenBlock<'a> for Html<'a> {
        type Closed = super::Html<'a>;

        fn stage(&mut self, line: &'a str) -> Result<&'a str, ()> {
            match self {
                Self::Case1(case) => case.stage(line).map_err(|_| ()),
                Self::Case2(case) => case.stage(line).map_err(|_| ()),
                Self::Case3(case) => case.stage(line).map_err(|_| ()),
                Self::Case4(case) => case.stage(line).map_err(|_| ()),
                Self::Case5(case) => case.stage(line).map_err(|_| ()),
                // Self::Case6(case) => case.stage(line).map_err(|_| ()),
            }
        }

        fn commit(&mut self) {
            match self {
                Self::Case1(case) => case.commit(),
                Self::Case2(case) => case.commit(),
                Self::Case3(case) => case.commit(),
                Self::Case4(case) => case.commit(),
                Self::Case5(case) => case.commit(),
                // Self::Case6(case) => case.commit(),
            }
        }

        fn close<F: FnMut(Self::Closed) -> ()>(self, mut sink: F) {
            match self {
                Self::Case1(case) => case.close(|case| sink(case.into())),
                Self::Case2(case) => case.close(|case| sink(case.into())),
                Self::Case3(case) => case.close(|case| sink(case.into())),
                Self::Case4(case) => case.close(|case| sink(case.into())),
                Self::Case5(case) => case.close(|case| sink(case.into())),
                // Self::Case6(case) => case.close(|case| sink(case.into())),
            }
        }
    }
}
...
mod parse {
    ...
    use parser::{
        Map, ParseResult, Parser, any_tag, empty, is_one_of, one_of, recognize, tag, take,
        take_while, validate,
    };
    ...
    mod utils {
        ...
        use parser::{
            IsEmpty, ItemsIndices, ParseResult, Parser, SplitAt, SubsetRange, is_one_of, maybe,
            not, one_of, recognize, repeated, tag, take, take_while, validate,
        };
