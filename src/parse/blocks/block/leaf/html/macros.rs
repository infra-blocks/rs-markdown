...
pub mod parse {
        ...
        ($valid:expr, $invalid:expr) => {
            ...
            mod test {
                ...
                fn should_work_with_text_before_tag() {
                    let text = concat!("some text before ", $valid);
                    assert_eq!(Ok(("", Closing::new(text))), closing(text),);
                }

                #[test]
                fn should_work_with_text_after_tag() {
                    let text = concat!($valid, " some text after\n");
                    assert_eq!(Ok(("", Closing::new(text))), closing(text),);
                }
            }
        };
    ...
    macro_rules! closing_module_macro {
        ($predicate:expr, $valid:expr, $invalid:expr) => {
            mod closing {
                use crate::parse::blocks::block::macros::parse::{
                ...
                closing_tests_macro!($valid, $invalid);
            }
        };
    }
