    ...
    T: NomParse<'a>,
{
    fn parse<I: Input<Item = &'a str>>(input: I) -> ParseResult<I, Self> {
        match Self::nom_parse::<Error<&str>>(input.segment()) {
            Ok((remaining, parsed)) => {
                if remaining.is_empty() {
                    input.parsed(ParseQuantity::Segments(1), parsed)
                } else {
                    let bytes_remaining = remaining.len();
                    let bytes_consumed = input.segment().len() - bytes_remaining;
    ...
    U: Parse<T> + Debug,
{
    fn strict_parse<I: Input<Item = T> + Debug>(input: I) -> Self {
        ...
        assert!(
            remaining.is_empty(),
            "remaining input after strict parse: {:?}",
            remaining
        );
        parsed
    }
