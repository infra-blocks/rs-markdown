    ...
    mod parse {
        ...
        macro_rules! failure_case {
            ($test:ident, $segment:expr) => {
                #[test]
                fn $test() {
                    assert!(UnbracketedLinkDestination::parse::<Error<&str>>($segment).is_err());
                }
            };
        }
        ...
        macro_rules! success_case {
            ...
            ($test:ident, $segment:expr, $parsed:expr, $remaining:expr) => {
                ...
                fn $test() {
                    assert_eq!(
                        UnbracketedLinkDestination::parse::<Error<&str>>($segment),
                        Ok(($remaining, UnbracketedLinkDestination::new($parsed)))
                    );
                }
            };
        }
