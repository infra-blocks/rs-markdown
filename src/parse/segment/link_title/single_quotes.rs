...
pub enum SingleQuotesLinkTitleSegments<'a> {
    Single(SingleQuotesLinkTitleSingleSegment<'a>),
    Multi(SingleQuotesLinkTitleMultiSegments<'a>),
}

impl<'a> Parse<'a> for SingleQuotesLinkTitleSegments<'a> {
...
mod test {
    ...
    mod parse {
        ...
        mod linke_title {
            ...
            test_parse_macros!(SingleQuotesLinkTitleSegments);

            failure_case!(should_reject_empty, "");
            failure_case!(should_reject_blank_line, " \t\n");

            success_case!(should_accept_single_segment, "'Hello!'\n", parsed => SingleQuotesLinkTitleSegments::Single(SingleQuotesLinkTitleSingleSegment::new("'Hello!'")), "\n");
            success_case!(should_accept_multi_segments, "'Hello,\nWorld!'\n", parsed => SingleQuotesLinkTitleSegments::Multi(SingleQuotesLinkTitleMultiSegments::new(
