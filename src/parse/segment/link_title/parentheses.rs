...
pub enum ParenthesesLinkTitleSegments<'a> {
    Single(ParenthesesLinkTitleSingleSegment<'a>),
    Multi(ParenthesesLinkTitleMultiSegments<'a>),
}

impl<'a> Parse<'a> for ParenthesesLinkTitleSegments<'a> {
...
mod test {
    ...
    mod parse {
        ...
        mod linke_title {
            ...
            test_parse_macros!(ParenthesesLinkTitleSegments);

            failure_case!(should_reject_empty, "");
            failure_case!(should_reject_blank_line, " \t\n");

            success_case!(should_accept_single_segment, "(Hello!)\n", parsed => ParenthesesLinkTitleSegments::Single(ParenthesesLinkTitleSingleSegment::new("(Hello!)")), "\n");
            success_case!(should_accept_multi_segments, "(Hello,\nWorld!)\n", parsed => ParenthesesLinkTitleSegments::Multi(ParenthesesLinkTitleMultiSegments::new(
